...
public class ArithmeticExam {
	...
	public static void main(String[] args) {
		...
		int result5 = v1 % v2 ;
		System.out.println("result5 = "+result5); //나머지 1
		
		double result6 = v1 / v2 ;
		System.out.println("result6 = "+result6); //2.0
		
		double result7 = (double) v1 / v2 ; //강제타입변환(v1)+자동타입변환(v2)
		...
		long zl = (long) x * y ;
		System.out.println("long 강제+자동타입변환 = "+zl);
		...
		double result = apple -(piece*pieceUnit) ;  // 1-(7*0.1)=0.3
		
		System.out.println("남은 사과 조각 수는 "+result+" 조각"); 
		//남은 사과 조각 수는 0.29999999999999993
		//결론 : 돈 계산 할 때는 double과 같은 실수형으로 사용하지 말 것!!!
		
		System.out.println("-------------------------");
		
		int appleInt = 1 ; //사과 1개
		int totalPieces = appleInt*10 ; //10등분으로 나누지말고 곱
		int number = 7 ; //사과 7조각
		int temp = totalPieces - number ; //사과 10조각 - 7조각 = 3조각
		
		double resultInt = temp / 10.0 ; // 3/10.0 ->0.3
		
		System.out.println("사과 1개에서 0.7조각을 빼면 "+resultInt+" 조각"); 
		...
		System.out.println("사회점수 : "+sol); 
		System.out.println("총점 : "+total);
		System.out.println((90+80)/2+" : 평균");
		// 출력시 앞 글자가 문자이면 뒤에 숫자는 자동 타입 변환으로 문자열 출력
		// 출력시 앞 글자가 계산정보이면 계산 후 문자열이 붙는다.
